...
EXPERIMENTS = [
    ...
    {"id": "E56", "title": "HorizonCache -- causal safe-horizon prediction (fresh/cache/jump) for flow-model caching",
     ...
     "result": "SIGNIFICANCE run (N=24 prompts x 2 seeds = 48 paired samples, 512px/28 steps; lean set full + SeaCache + "
               "adaptive_1.25 only; tau grid 0.2/0.3/0.4/0.5/0.65; bootstrap 95% CI over prompt-seed pairs). Preceded by an N=4 "
               "smoke (~+0.5 dB) and an N=20 single-seed consolidation. The SURVIVING PRIMITIVE is the CONSERVATIVE ADAPTIVE jump "
               "(jf=1+(jf_max-1)*headroom capped at 1.25; it beats fixed regrid_1.25). At MATCHED ACHIEVED speedup the multi-seed "
               "result is ROBUST and every in-band CI EXCLUDES ZERO: +1.61 dB@1.70x (CI[1.15,2.10],win85%), +1.47@2.11x "
               "([0.95,2.04],83%), +2.11@2.51x ([1.52,2.70],92%) -- the strong-KEEP-band point -- +1.26@2.74x ([0.81,1.68],92%); "
               "SeaCache frontier 35.6/30.4/28.5/25.9/23.1 dB @1.48/1.86/2.11/2.48/3.0x. Mechanism (now clear): the jump lets "
               "HorizonCache keep refreshing OFTEN (low tau) yet still save compute -> a gentler quality/speed tradeoff than "
               "SeaCache's rare-refresh/long-cache, exactly where SeaCache's frontier drops steepest. NARROW-BAND: OVERSHOOTS past "
               "~3x (tau0.65: -0.19 dB @3.40x, CI[-0.25,-0.13] excl 0 on the NEGATIVE side, win 19%). jump_2.0 = KILL. "
               "v1 LABEL FINDING (the methodological crux): the safe-horizon label is two-dimensional (metric x horizon). "
               "Strict latent-L2 (2%) labels 0 safe jumps (too pessimistic). Short-H=5 decoded-PSNR (floor 32) labels 12 but 11/12 "
               "are jump_2.0 -- which KILLS end-to-end quality: the short continuation is too OPTIMISTIC (misses compounding, the "
