...
echo "== STAGE 1: E61 CONSOLIDATION — N=100 x2 seeds, tau 0.3..1.4 =="
OUT="/storage/nada/colorful-noise-e61/results/horizon_gate_consol"
START=$(date -Is)
python -m horizon_cache.run --mode generation   --n 100 --seeds-per-prompt 2 --seed-base 0   --steps 28 --width 512 --height 512 --guidance 3.5   --dtype bf16 --lean --save-all-images   --tau-grid 0.3 0.4 0.5 0.575 0.65 0.8 1.0 1.2 1.4   --variants adaptive_1.25 rmraw0.5_adaptive_1.25 rmcl0.5_adaptive_1.25 rmgh0.85_adaptive_1.25 rmgh0.75_adaptive_1.25   --rm-lambda-mode sigma --rm-lambda-max 1.5   --jump-mode regrid   --out "$OUT"
RC=$?
END=$(date -Is)
...
tar -C "$LATEST" -czf /storage/nada/colorful-noise-e61/e61_consol_light.tar.gz metrics.csv metrics.json summary.json config.json traces 2>/dev/null || true
tar -C "$LATEST" -czf /storage/nada/colorful-noise-e61/e61_consol_samples.tar.gz samples 2>/dev/null || true
ls -la /storage/nada/colorful-noise-e61/e61_consol_light.tar.gz 2>/dev/null || true
echo "== DONE_MARKER rc=$RC consol=$LATEST =="
